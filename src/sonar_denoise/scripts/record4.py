...
1. 声呐图像处理: /son/compressed -> /sonar_oculus_node/ping
   - 修改: 不使用 /son/compressed 中的图像，而是从 /home/hzr/rosbag/Aracati_full_sonargan 文件夹中根据时间戳读取对应的 PNG 图像。
     例如，时间戳 123456789.123456789 秒对应 123456789_123456789.png。
2. 运动数据处理:
...
OUTPUT_BAG_PATH = '/home/hzr/rosbag/ARACATI_2017_8bits_full_sonargan_fake.bag'
TIME_SYNC_THRESHOLD = 0.1  # /cmd_vel 和 /pose_gt 之间允许的最大时间差 (秒)
SONARGAN_FOLDER = '/home/hzr/rosbag/Aracati_full_sonargan_fake'

# --- 声呐图像转换函数 ---
def process_son_compressed(image, header, ping_id_counter):
    try:
        ...
        oculus_msg = OculusPingUncompressed(
            header=header,
            fire_msg=OculusFire(header=header, mode=1, gamma=128, flags=0, range=max_range, gain=0.0, speed_of_sound=1500.0, salinity=35.0),
            ping_id=ping_id_counter,
            part_number=1032,
            start_time=header.stamp.secs,
        ...
        with tqdm(total=total_messages, desc="Processing ROS bag", unit="msg") as pbar:
            for topic, msg, t in rosbag.Bag(INPUT_BAG_PATH).read_messages():
                if topic == '/son/compressed':
                    ...
                    filename = f"{time_sec}_{time_nsec}.png"
                    image_path = os.path.join(SONARGAN_FOLDER, filename)
                    ...
                    if os.path.exists(image_path):
                        ...
                        if image is None:
                            ...
                            oculus_msg, ping_id_counter = process_son_compressed(image, msg.header, ping_id_counter)
                            if oculus_msg:
                                outbag.write('/sonar_oculus_node/ping', oculus_msg, t)
                    else:
                        rospy.logwarn("Image file not found: %s", image_path)
